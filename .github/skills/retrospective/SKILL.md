...
Conventions, naming rules, lint patterns, framework idioms. Add `applyTo` glob frontmatter so it auto-loads for matching files.

### 2e. Is it a multi-step workflow? → `skills/<name>/SKILL.md`

A repeatable procedure with multiple steps (release prep, debugging flow, new-component scaffolding).
